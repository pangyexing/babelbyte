...
| 3      | Twitter 短推  | `source=twitter AND len < 200`           | 有高价值信号例外 |
| 4      | Reddit 链接帖 | `len < 100 AND "[link]" in content`      | 纯链接无价值     |
| 5      | 转推          | `title.startswith("rt @") AND len < 500` | 平均重要性仅 4.8 |
| 6      | 艺术帖        | 匹配艺术模式                             | 与技术无关       |
| 7      | 招聘帖        | 匹配 10+招聘模式                         | 不属于内容范围   |
| 8      | 垃圾/推广     | 匹配 4 类垃圾模式                        | 低质量           |
| 9      | 跨发帖        | "cross-posted"/"xpost"                   | 重复内容         |
| 10     | 机器人        | 3 种 bot 标记                            | 自动生成         |
...
| 最小关键字 boost | `RULE_ONLY_MIN_BOOST`   | 1      |
...
### 1.3 `find_similar_processed_item()` - Embedding 语义去重
...
| `MODEL_TIER_HEAVY_THRESHOLD`       | 7      | 分数>=7 用重型    |
| `MODEL_TIER_CONFIDENCE_CUTOFF`     | 0.5    | 置信度<0.5 用重型 |
...
| HN 来源    | 5     | 0.6        | source:hackernews | LIGHT    |
...
> 本节分析单卡 4090 (24GB VRAM) 环境下 Ollama 并行处理的配置优化。

### 背景

Two-stage 处理流程使用 ThreadPoolExecutor 实现并行：

**文件**: `src/processors/ollama_api.py:766-1132`

```
Stage 1: 8B screening  → ThreadPoolExecutor (workers_screen)
Stage 2: 32B refinement → ThreadPoolExecutor (workers_heavy)
```

### 为什么单卡并行仍然有效？

直觉上，单卡 GPU 应该串行处理更快，但实测**并行更快**，原因如下：

#### 1. Continuous Batching（连续批处理）

Ollama 支持将多个并发请求的 token 生成合并成批次处理：

```
串行模式：
[请求1: 生成所有token] → [请求2: 生成所有token] → [请求3: 生成所有token]
                      ↑ 等待             ↑ 等待

并行模式 (OLLAMA_NUM_PARALLEL > 1)：
[请求1+2+3 的 token 交替生成，GPU 持续满载运算]
```

单次生成一个 token 的计算量可能无法占满 4090 的全部 CUDA cores，批处理多个请求可以提高 GPU 利用率。
...
| 模型            | 显存占用 | 单请求 GPU 利用率 |
| --------------- | -------- | ----------------- |
| 8B (qwen3:8b)   | 5-8 GB   | ~60-70%           |
| 32B (qwen3:32b) | 16-20 GB | ~90-95%           |

8B 模型的 2-4 个并行请求可以更充分利用 CUDA cores，显存也足够容纳多个 KV cache。

#### 3. I/O 延迟隐藏

```
串行：[GPU计算] → [等待网络响应] → [准备下个请求] → [GPU计算]
                     ↑ GPU 空闲

并行：[GPU计算] ←→ [下个请求准备/网络传输同时进行]
                     ↑ GPU 持续工作
```
